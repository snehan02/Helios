...
const ClientView = () => {
    ...
    return (
        <div className="min-h-[calc(100vh-8rem)] bg-main-gradient p-4">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 h-full">
                {/* Main Content: Calendar */}
                <div className="lg:col-span-2 flex flex-col group">
                    <div className="mb-10 flex flex-col md:flex-row md:justify-between md:items-end gap-6 glass-panel p-8 relative overflow-hidden">
                        ...
                        <div className="w-full md:w-auto">
                            <div className="flex items-center gap-3 mb-2">
                                <div className="w-2 h-2 rounded-full bg-zinc-400 dark:bg-white shadow-[0_0_10px_rgba(255,255,255,0.8)]" />
                                <h1 className="text-2xl md:text-3xl font-black text-zinc-900 dark:text-white tracking-tighter uppercase break-words">{client?.name || 'Loading...'}</h1>
                            </div>
                            <p className="text-zinc-500 font-medium text-sm">
                                Managing project status and resources for <span className="text-zinc-700 dark:text-zinc-300">partner view</span>.
                            </p>
                        </div>
                ...
                <div className="flex flex-col gap-6 lg:h-full lg:overflow-y-auto custom-scrollbar pr-2">
                            ...
                            {isLayoutMode && (
                                <div className="absolute -top-3 -right-3 z-10 flex gap-1">
                                    ...
                                    <button
                                        ...
                                        className="p-1.5 bg-rose-500/10 hover:bg-rose-500 border border-rose-500/20 text-rose-600 hover:text-white rounded-lg transition-all"
                                    >
                    ...
                    {isLayoutMode && (
                        <button
                            ...
                            className="w-full py-6 border-2 border-dashed border-zinc-300 dark:border-zinc-700 rounded-2xl text-zinc-500 hover:text-zinc-900 dark:hover:text-white hover:border-zinc-400 dark:hover:border-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800/30 transition-all flex flex-col items-center justify-center gap-2 group"
                        >
                            <span className="text-3xl font-light group-hover:scale-125 transition-transform">+</span>
                            <span className="font-bold uppercase tracking-widest text-[10px]">Add Widget</span>
                        </button>
                    )}
                    ...
                    {showWidgetModal && (
                        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-md p-4">
                            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-3xl w-full max-w-sm shadow-2xl">
                                <h3 className="text-xl font-black text-zinc-900 dark:text-white mb-6 uppercase tracking-tight">Add New Widget</h3>
                                <div className="space-y-6">
                                    <div>
                                        <label className="block text-[10px] font-black uppercase tracking-wider text-zinc-500 mb-2">Widget Title</label>
                                        <input
                                            type="text"
                                            value={newWidgetTitle}
                                            onChange={(e) => setNewWidgetTitle(e.target.value)}
                                            className="w-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl p-3 text-zinc-900 dark:text-white focus:outline-none focus:border-zinc-900 dark:focus:border-white transition-colors"
                                            placeholder="e.g. Weekly Reports"
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-[10px] font-black uppercase tracking-wider text-zinc-500 mb-2">Type</label>
                                        <select
                                            value={newWidgetType}
                                            onChange={(e) => setNewWidgetType(e.target.value)}
                                            className="w-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl p-3 text-zinc-900 dark:text-white focus:outline-none focus:border-zinc-900 dark:focus:border-white transition-colors appearance-none"
                                        >
                                    ...
                                    <div className="flex gap-4 pt-4">
                                        <button
                                            ...
                                            className="flex-1 px-4 py-3 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 rounded-xl text-xs font-bold uppercase tracking-widest transition-colors"
                                        >
                                            Cancel
                                        </button>
                                        <button
                                            onClick={handleAddWidget}
                                            className="flex-1 px-4 py-3 btn-silver rounded-xl text-xs font-bold uppercase tracking-widest"
                                        >
                                            Add Widget
                                        </button>
                                    </div>
